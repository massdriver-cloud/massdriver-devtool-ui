...
const EnhancedConnectionsView = () => {
  ...
  const onSubmit = async () => {
    const requiredErrors = connectionTypes.reduce((acc, cur) => ({
      ...acc,
      ...(!formData[cur] ? {
        [cur]: 'Required'
      } : {})
    ...
    response?.ok ? enqueueSnackbar("Successfully Saved Connections", {
      variant: 'success',
      preventDuplicate: true,
      autoHideDuration: 5000,
      disableWindowBlurListener: true
    }) : enqueueSnackbar("There was an issue saving your connections", {
      variant: 'error',
      preventDuplicate: true,
      autoHideDuration: 5000,
      disableWindowBlurListener: true
